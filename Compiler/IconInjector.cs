...
namespace RSHELL_IDE.Compiler
{
    class IconInjector
    {
        ...
        public static void InjectIconFromByte(string exeFileName, byte[] iconFileName)
        {
            InjectIconFromByte(exeFileName, iconFileName, 1, 1);
        }
        ...
        public static void InjectIconFromByte(string exeFileName, byte[] iconFileName, uint iconGroupID, uint iconBaseID)
        {
            const uint RT_ICON = 3u;
            const uint RT_GROUP_ICON = 14u;
            IconFile iconFile = IconFile.FromByte(iconFileName);
            var hUpdate = NativeMethods.BeginUpdateResource(exeFileName, false);
            var data = iconFile.CreateIconGroupData(iconBaseID);
            NativeMethods.UpdateResource(hUpdate, new IntPtr(RT_GROUP_ICON), new IntPtr(iconGroupID), 0, data,
                data.Length);
